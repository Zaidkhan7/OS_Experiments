...
public class SJFNonpreemptive {
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		  ...
		  int i, smallest, count=0, time, n,j,temp1=0,min=0,sum=0;;
		 ...
		 Scanner in = new Scanner(System.in);
	      ...
	      System.out.println(at[0]+"\t\t\t"+bt[0]+"\t\t\t"+"0"+"\t\t\t"+bt[0]);
	      ...
	      while(at.length!=0)
	      {
	    	  for(j=0;j<at.length;j++)
		      {
		    	  ...
		    	  sum=sum+bt[min];
		    	  wt[min] = bt[min]+at[min];
		    	  tat[min] = wt[min]+bt[min];
		    	  System.out.println(at[min]+"\t\t\t"+bt[min]+"\t\t\t"+wt[min]+"\t\t\t"+tat[i]);
